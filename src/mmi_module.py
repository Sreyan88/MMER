...
class MMI_Model(nn.Module):
    """Coupled Cross-Modal Attention BERT model for token-level classification with CRF on top.
    """
    def __init__(self, config, ctc_output_size, label_output_size, alpha=0.1, layer_num1=1, layer_num2=1, layer_num3=1,  num_labels=2, auxnum_labels=2):
        #super(BertPreTrainedModel, self).__init__()
    ...
    def forward(self, bert_input_ids, bert_attention_mask, bert_segment_ids, text_length, audio_input, audio_length, ctc_labels, emotion_labels, text_output):
        ...
        loss = loss_cls + (0.001*loss_ctc)

        return loss, classification_feats_pooled, loss_cls, loss_ctc

class UNI_BASELINE(nn.Module):
    """Coupled Cross-Modal Attention BERT model for token-level classification with CRF on top.
    """
    def __init__(self, config, ctc_output_size, label_output_size, alpha=0.1, layer_num1=1, layer_num2=1, layer_num3=1,  num_labels=2, auxnum_labels=2):
        super(UNI_BASELINE, self).__init__()
...
class CAI_SOTA(nn.Module):
    """Coupled Cross-Modal Attention BERT model for token-level classification with CRF on top.
    """
    def __init__(self, config, ctc_output_size, label_output_size, alpha=0.1, layer_num1=1, layer_num2=1, layer_num3=1,  num_labels=2, auxnum_labels=2):
        super(CAI_SOTA, self).__init__()
        self.num_labels = num_labels
        self.wav2vec2 = Wav2Vec2Model.from_pretrained("facebook/wav2vec2-base",output_hidden_states=True,return_dict=True,apply_spec_augment=False)
        self.wav2vec2.feature_extractor._freeze_parameters()
